...
export default function EmployeeDashboard() {
  ...
  useEffect(() => {
    ...
    // Re-fetch leaves when user returns to the tab (fallback)
    const handleVisibility = () => {
      if (document.visibilityState === 'visible') fetchLeaves()
    }
    document.addEventListener('visibilitychange', handleVisibility)
    return () => document.removeEventListener('visibilitychange', handleVisibility)
  }, [profile])

  // ── Real-time leave status subscription ──
  useEffect(() => {
    if (!profile) return
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          // Instantly update the leave row when manager approves / rejects
          setLeaves(prev =>
            prev.map(l => l.id === payload.new.id ? { ...l, ...payload.new } : l)
          )
          const status = payload.new.status
          if (status === 'approved') toast.success('🎉 Your leave has been approved!')
  ...
  async function applyLeave(e) {
    ...
    const { error } = await supabase
      .from('leaves')
      .insert({ user_id: profile.id, ...leaveForm })
    if (error) {
                ...
                <div className="max-h-64 overflow-y-auto">
                  {notifications.length === 0
                    ...
                    : notifications.map(n => (
                      <div
                        key={n.id}
                        className={`px-4 py-3 border-b border-slate-50 text-sm ${!n.read ? 'bg-indigo-50' : ''}`}
                      >
        ...
        {activeTab === 'dashboard' && (
          <>
            ...
            <div className={`rounded-xl border-2 p-4 flex items-center justify-between ${
              reportSubmitted ? 'bg-emerald-50 border-emerald-200' : 'bg-yellow-50 border-yellow-200'
            }`}>
              <div>
              ...
              {reportSubmitted
                ...
                : (
                  <button
                    ...
                    className="text-xs bg-yellow-500 text-white px-3 py-1 rounded-full font-medium hover:bg-yellow-600 transition"
                  >
                    Submit Now
                  </button>
                )}
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => setActiveTab('tasks')} className="text-xs text-indigo-600 hover:underline">
                  View all
                </button>
              </div>
                  ...
                  <div className="space-y-3">
                    {leaves.map(leave => (
                      ...
                      >
                        ...
                        <span className={`text-xs px-2 py-1 rounded-full font-medium shrink-0 ${LEAVE_COLORS[leave.status]}`}>
                          {leave.status}
                        </span>
                      </div>
                    ))}
